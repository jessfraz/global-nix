  ...
  programs.git = {
    ...
    settings = {
      ...
      alias = {
        ...
        wtnew = "!f() { b=\"$1\"; if [ -z \"$b\" ]; then echo \"usage: git wtnew <branch>\"; return 1; fi; repo=$(basename \"$(git rev-parse --show-toplevel)\"); wt=\"../$repo-$b\"; git worktree add -b \"$b\" \"$wt\" origin/main || return $?; git -C \"$wt\" submodule update --init --recursive || return $?; if git show-ref --verify --quiet \"refs/remotes/origin/$b\"; then git -C \"$wt\" branch --set-upstream-to=\"origin/$b\" \"$b\"; else echo \"origin/$b does not exist yet. Run: git -C $wt push -u origin $b\"; fi; cd \"$wt\"; }; f";
